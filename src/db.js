...
// 旧DBからの移行で足りない列があれば静かに追加（あってもエラー無視）
try { db.exec('ALTER TABLE users ADD COLUMN win_streak INTEGER DEFAULT 0'); } catch {}
try { db.exec('ALTER TABLE users ADD COLUMN points INTEGER DEFAULT 300'); } catch {}
...
try { db.exec('ALTER TABLE users ADD COLUMN guild_id TEXT'); } catch {}
...
INSERT INTO users (guild_id, user_id, username, points, wins, losses, win_streak)
...
// ===== signup =====
export const createSignup = db.prepare(`
INSERT INTO signup (guild_id, message_id, channel_id, author_id, created_at)
VALUES (?, ?, ?, ?, ?)
`);

export const latestSignupMessageId = db.prepare(`
SELECT message_id FROM signup
WHERE guild_id=?
...
DELETE FROM signup_participants WHERE guild_id=? AND message_id=? AND user_id=?
`);
...
export const clearParticipantsByMessage = db.prepare(`
DELETE FROM signup_participants WHERE guild_id=? AND message_id=?
`);

// ===== matches =====
export const createMatch = db.prepare(`
INSERT INTO matches (guild_id, message_id, team_a, team_b, created_at)
VALUES (?, ?, ?, ?, ?)
`);

export const getLatestMatch = db.prepare(`
SELECT * FROM matches
WHERE guild_id=?
ORDER BY id DESC LIMIT 1
`);

export const getMatchById = db.prepare(`
SELECT * FROM matches
WHERE id=? AND guild_id=?
`);

export const setMatchWinner = db.prepare(`
UPDATE matches SET winner=? WHERE id=? AND guild_id=?
`);
...
export const getLastSignature = db.prepare(`
SELECT signature FROM last_team_signature WHERE guild_id=?
`);
