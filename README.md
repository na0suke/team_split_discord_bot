...
LoL のように「TOP / JG / MID / ADC / SUP」で役割を指定して参加できる新方式です。  
役割が被らないように、かつ強さ（ポイント）を考慮してチームを自動編成します。  

### コマンド
- `/start_lane_signup`  
  ポジション別の参加受付を開始。  
  メッセージに以下のリアクションが付与されます：  
  - ⚔️ TOP  
  - 🌲 JG  
  - 🪄 MID  
  - 🏹 ADC  
  ...
  ✅ が押されると、参加者からチームを編成し、各チームに **通し番号のチームID** が付与されます。  

- `/result_team winteam:<id> loseteam:<id>`  
  指定したチームID同士の試合結果を登録します。  

### ポイント計算（レーン試合専用）
- 勝利：**+6**  
- 敗北：**-4**  
- 連勝ボーナス：2 点刻み（2連勝=+2, 3連勝=+4 …）  
- 連敗ペナルティ：2 点刻み（2連敗=-2, 3連敗=-4 …）  

### 特徴
- 2チーム以上の同時編成に対応（例：各レーン3人ずついれば3チーム作成）  
- チームは「チームID」で管理され、勝敗登録時はこの ID を指定します  
- 通常試合（/team, /result）とは別ルールでポイント計算されます  

---

## 🖼 Embed 表示例（レーン指定）

### ⚔️ レーン分け（`/start_lane_signup` → ✅ 実行時）
**チーム分け結果（レーン指定） — TeamID #101, #102**
